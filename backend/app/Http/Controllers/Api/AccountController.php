...
class InstallmentController extends Controller
{
    ...
    public function payInstallment(Request $request)
    {
        ...
        try {
            ...
            // ============================================
            // ✅ FIX: Account ka paid_amount RECALCULATE karo
            // Installments table se sum karke taake advance payment bhi reflect ho
            // ============================================
            $account = Account::find($installment->account_id);
            if ($account) {
                // Installments table se total paid amount calculate karo
                $totalInstallmentPaid = Installment::where('account_id', $account->id)->sum('paid_amount');
                
                // Account ka paid_amount = installment paid sum (jo advance bhi include karega)
                $newAccountPaid = $totalInstallmentPaid;
